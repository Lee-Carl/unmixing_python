...
class Model(nn.Module):
    def __init__(self, L, P):
        ...
        self.layer1 = nn.Sequential(
            ...
            nn.Conv2d(self.L, 256, self.kernel_sizes[0], stride=self.strides[0], padding=0),
            nn.BatchNorm2d(256),
            nn.LeakyReLU(**self.lrelu_params),
        )

        self.layer2 = nn.Sequential(
            # nn.ReflectionPad2d(self.padding[1]), # 1
        ...
        self.layer3 = nn.Sequential(
            nn.BatchNorm2d(260),
            # nn.ReflectionPad2d(self.padding[2]), # 2
            nn.Conv2d(260, 256, self.kernel_sizes[2], stride=self.strides[2], padding=1),
            nn.BatchNorm2d(256),
            nn.LeakyReLU(**self.lrelu_params),
        )
