...
class Model(nn.Module):
    def __init__(self, L, P):
        ...
        self.layer1 = nn.Sequential(
            ...
            nn.Conv2d(self.L, 256, self.kernel_sizes[0], stride=self.strides[0],padding=1),
            # nn.Conv2d(self.L, 256, 3, 1),
            nn.BatchNorm2d(256),
            nn.LeakyReLU(**self.lrelu_params),
            # layer2
            # nn.ReflectionPad2d(self.padding[1]),
            nn.Conv2d(256, 256, self.kernel_sizes[1], stride=self.strides[1],padding=1),
            nn.BatchNorm2d(256),
            nn.LeakyReLU(**self.lrelu_params),
    ...
    @staticmethod
    def up(x, upscale=2):
        return x[:, :, :, None, :, None].expand(-1, -1, -1, upscale, -1, upscale) \
    ...
    def forward(self, x, i):
        ...
        # x_skip = self.layerskip(x)
        # x_cat = self.custom_cat(x_un, x_skip)
        ...
        if i == 0:
            print(x5.shape)
        if i == 99:
            # print(x_un.shape, torch.sum(x_un))
            # print(x_skip.shape, torch.sum(x_skip))
            # print(x_cat.shape,torch.sum(x_cat))
