...
from .model import PGMSU2
from methods.Base import Base
from utils import restraint
...
class DGA(Base):
    def __init__(self, params, init):
        super(DGA, self).__init__(params=params, init=init)
    ...
    def run(self, savepath=None, output_display=True, *args, **kwargs):
        ...
        # 生成DataLoader
        Y = Y.T
        train_db = torch.utils.data.TensorDataset(torch.tensor(Y), torch.tensor(A_true.T))
        ...
        # 模型设置
        model = PGMSU2(P, Channel, z_dim).to(device)
        ...
        for epoch in range(epochs):
            model.train()
            for step, (y, abu) in enumerate(train_db):
                y = y.to(device)
                ...
                if epoch < epochs // 2:
                    ...
                    Loss_Sp = restraint.norm_l1_2(a, lambda_sp)  # L1/2约束
                    Loss_SAD = lambda_sad * loss_sad  # 光谱角距
                    Loss_Minvol = lambda_vol * loss_minvol  # 最小体积约束
                    Loss_KL = lambda_kl * kl_div  # KL散度
                    # Loss_A = ((abu.to(device) - a) ** 2).sum() / a.shape[0]
                    loss = Loss_Rec + Loss_Sp + Loss_SAD + Loss_Minvol + Loss_KL

                    # if epoch == epochs - 1 and output_display:
                    #     theloss = f'loss_sad:{loss_sad} | loss_minvol:{loss_minvol} | kl_div:{kl_div} | l12_norm:{restraint.l12_norm(a)} | Loss_Rec:{Loss_Rec}'
                    #     print(theloss)

                if (epoch + 1) % 10 == 0 and output_display:
                    print(f'loss = {loss}')
        ...
        with torch.no_grad():
            ...
            return {'E': em_hat.data.cpu().numpy(),
                    'A': A_hat.T,
                    'Y': y_hat.cpu().numpy()}
